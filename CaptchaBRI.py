...
def solve_captcha(path):
    ...
    th, threshed1 = cv.threshold(image,1,255,cv.THRESH_BINARY)
    th, threshed2 = cv.threshold(image,10,255,cv.THRESH_BINARY)
    th, threshed3 = cv.threshold(image,20,255,cv.THRESH_BINARY)
    th, threshed4 = cv.threshold(image,25,255,cv.THRESH_BINARY)

    filename = "{}.png".format(os.getpid())
    #cv.imwrite(filename,threshed4)
    #image.save(filename)
